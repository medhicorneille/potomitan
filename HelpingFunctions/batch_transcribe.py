...
def main(start_datetime, end_datetime):
    ...
    for filename in tqdm(os.listdir(AUDIO_DIR)):
        if (filename.endswith(".wav") or filename.endswith(".mp3")) and filename not in existing:
            ...
            if start_datetime <= file_creation_datetime <= end_datetime:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_datetime', type=str, required=True, help='Start datetime of files to transcribe (YYYY-MM-DD HH:MM:SS)')
    parser.add_argument('--end_datetime', type=str, required=True, help='End datetime of files to transcribe (YYYY-MM-DD HH:MM:SS)')
